...
class HQSGTRD {
 public:
  ...
  const int N8[8][2] = {{0, -1},  {-1, 0}, {0, 1}, {1, 0},
  ...
  void Superpixel_Segmentation(string image_file_name, string edge_file_name,
                               int Superpixel_Num, string file_name,
                               vector<double>& run_time, int iteration_num,
                               int Save_Image, int Save_Label_file);

  void Calculate_Contour_Information(Mat LabImage,
                                     vector<vector<int>>& t_value);

  void RearC(Mat image, int start, vector<vector<pixelsPoinT>>& PixelMatrixT,
             vector<vector<int>> t_value);

  void middleC(Mat image, int start, vector<vector<pixelsPoinT>>& PixelMatrixT,
               vector<vector<int>> t_value);
  ...
  int Superpixel_Generation(Mat Image, int Superpixel_Num,
                            Mat& Finally_labelMat, int iteration_num);

  void Flat_Regions_Generation(Mat img, int Superpixel_Num, int cstrips,
                               double SuperPixel_size, double grid_interval_S);

  void ProduceSuperpixelLable(int x, int y, int m, int SeedX, int SeedY,
                              int& SuperPixelsNum);
  ...
  int Non_Flat_Regions_Generation(string path, int superpixel_num_K,
                                  int grid_interval_S, Mat img,
                                  int iteration_num, Mat& Finally_labelMat);

  void Label2Boundary(int* label, Mat img, Mat img_boundary);

  void Iterative_Generation(cv::Mat& labelMat, int numseeds,
                            int grid_interval_S, int itr_times, cv::Mat img);

  void Weights_Adjustment(cv::Mat labelMat_sign, cv::Mat Label, double& WL,
                          double& Wa, double& Wb, double& Wg, double& Ws,
                          centroid_Cluster coC, int numseeds);

  void Update_Seed_Point(cv::Mat Label, cv::Mat img_lab, int num_region,
                         centroid_Cluster coC, int* spixel_size);

  void Superpixel_Combination(cv::Mat labelMat);

  void number_Statistical_SuperpixelsLable(int CNum, Nodem NumberSuperPixle);

  void CombineSuperpixelsPriority(int countNum, int& NumberSuperpixels,
                                  int length, Nodem NumberSuperPixle);

  void CombineSuperpixelsPriorityS(int countNum, int& NumberSuperpixels,
                                   int length, Nodem NumberSuperPixle);
