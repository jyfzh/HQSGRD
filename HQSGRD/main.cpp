...
void getAllFiles(std::string path, std::vector<fs::path>& files, std::string filetype){
...
int main() {
	...
	for (int Superpixel_Num = 100; Superpixel_Num <= 1000; Superpixel_Num += 100)
	{
		for (auto input_file : input_files)
		{
			...
			HQSGTRD sp;
			sp.Superpixel_Segmentation(input_file, RCF_file, Superpixel_Num, output_file, run_time, iteration_num,  Save_Image, Save_Label_file);
		}
		double time = 0;
