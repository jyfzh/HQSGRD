...
void HQSGTRD::Superpixel_Segmentation(string image_file_name,
                                      string edge_file_name, int Superpixel_Num,
                                      string file_name,
                                      vector<double>& run_time,
                                      int iteration_num, int Save_Image,
                                      int Save_Label_file) {
      ...
      if ((int)imgEge.at<uchar>(i, j) > 2) {  //&& we[i][j]<7) {
        ...
        for (int n = i - 2; n <= i + 1; n++) {
          for (int m = j - 2; m <= j + 1; m++) {
            ...
            Enhance_boundaries.at<uchar>(n, m) =
                imgEge.at<uchar>(i, j);  // % 255;
          }
        }
      } else {
  ...
  int final_label = Superpixel_Generation(imgEge, Superpixel_Num,
                                          Finally_labelMat, iteration_num);
...
void HQSGTRD::Calculate_Contour_Information(Mat LabImage,
                                            vector<vector<int>>& t_value) {
  ...
  for (int i = 0; i < LabImage.rows; i++) {
    vector<int> W_temp;
    for (int j = 0; j < LabImage.cols; j++) {
      Edge_enhanced_L_channel[i][j] =
          PixelMatrix[i][j].L + (int)LabImage.at<uchar>(i, j);
    }
  }
  ...
  for (int i = 1; i < LabImage.rows - 1; i++) {
    for (int j = 1; j < LabImage.cols - 1; j++) {
      v2 = (Edge_enhanced_L_channel[i - 1][j] +
            Edge_enhanced_L_channel[i - 1][j - 1] +
            Edge_enhanced_L_channel[i - 1][j + 1]) -
           (Edge_enhanced_L_channel[i + 1][j] +
            Edge_enhanced_L_channel[i + 1][j - 1] +
            Edge_enhanced_L_channel[i + 1][j + 1]);

      g2 = (Edge_enhanced_L_channel[i - 1][j - 1] +
            Edge_enhanced_L_channel[i][j - 1] +
            Edge_enhanced_L_channel[i + 1][j - 1]) -
           (Edge_enhanced_L_channel[i + 1][j + 1] +
            Edge_enhanced_L_channel[i][j + 1] +
            Edge_enhanced_L_channel[i - 1][j + 1]);
  ...
  for (int i = 1; i < LabImage.rows - 1; i++) {
    for (int j = 1; j < LabImage.cols - 1; j++) {
      ...
      /*if (PixelMatrixNew[i][j].g > 0) {
                  ...
                  img56.at<Vec3b>(i, j)[0] = (int)PixelMatrixNew[i][j].g + 40;//
         % 255; img56.at<Vec3b>(i, j)[1] = (int)PixelMatrixNew[i][j].g + 40;// %
         255; img56.at<Vec3b>(i, j)[2] = (int)PixelMatrixNew[i][j].g + 40;// %
         255;
                  }
                  else
                  {
                  img56.at<Vec3b>(i, j)[0] = (int)PixelMatrixNew[i][j].g;// %
         255; img56.at<Vec3b>(i, j)[1] = (int)PixelMatrixNew[i][j].g;// % 255;
                  img56.at<Vec3b>(i, j)[2] = (int)PixelMatrixNew[i][j].g;// %
         255;
                  }
                  */
    }
...
void forwardC(Mat image, int end, vector<vector<pixelsPoinT>>& PixelMatrixT,
              vector<vector<int>> t_value) {
  int B[9];
  int CCCnum = 0;
  for (int i = 0; i < 9; i++) {
        ...
        for (int J = j - 1; J <= j + 1; J++) {
          ...
          if (PixelMatrixT[i][j].D[u] == -1) {
            PixelMatrixT[i][j].D[u] = round((double)sqrt(
                (PixelMatrixT[iNum][jNum].R - PixelMatrixT[I][J].R) *
                    (PixelMatrixT[iNum][jNum].R - PixelMatrixT[I][J].R) +
                (PixelMatrixT[iNum][jNum].G - PixelMatrixT[I][J].G) *
                    (PixelMatrixT[iNum][jNum].G - PixelMatrixT[I][J].G) +
                (PixelMatrixT[iNum][jNum].B - PixelMatrixT[I][J].B) *
                    (PixelMatrixT[iNum][jNum].B - PixelMatrixT[I][J].B)));
...
void HQSGTRD::RearC(Mat image, int start,
                    vector<vector<pixelsPoinT>>& PixelMatrixT,
                    vector<vector<int>> t_value) {
  int B[9];
  int CCCnum = 0;
  for (int i = 0; i < 9; i++) {
...
void HQSGTRD::middleC(Mat image, int start,
                      vector<vector<pixelsPoinT>>& PixelMatrixT,
                      vector<vector<int>> t_value) {
  int B[9];
  int CCCnum = 0;
...
void HQSGTRD::Generate_Classification(
    vector<vector<pixelsPoinT>>& PixelMatrixT) {
        ...
        for (int J = jNum - 1; J <= jNum + 1; J++) {
          ...
          if (PixelMatrix[I][J].ClassSign != -1) {
            if (PixelMatrix[iNum][jNum].ClassSign != -1) {
              if (PixelMatrix[iNum][jNum].ClassSign !=
                  PixelMatrix[I][J].ClassSign) {
                if (PixelMatrix[iNum][jNum].ClassSign >
                    PixelMatrix[I][J].ClassSign) {
                  if (classificationLable ==
                      PixelMatrix[iNum][jNum].ClassSign) {
                    classificationLable--;
                  }
...
int HQSGTRD::Superpixel_Generation(Mat Image, int Superpixel_Num,
                                   Mat& Finally_labelMat, int iteration_num) {
  ...
  Flat_Regions_Generation(Image, Superpixel_Num_Actually, cstrips, spixel_size,
                          grid_interval_S);
  int final_label = Non_Flat_Regions_Generation(
      FileName, Superpixel_Num_Actually, grid_interval_S, Image, iteration_num,
      Finally_labelMat);

  return final_label;
}

inline void HQSGTRD::Flat_Regions_Generation(Mat img, int Superpixel_Num,
                                             int cstrips, double spixel_size,
                                             double grid_interval_S) {
  int SuperPixelsNum = 0;

  for (int s = 0; s < Superpixel_Num; s++) {
    SuperPixelsNum = 0;
    ProduceSuperpixelLable(Seed_Point[s].Modified_x, Seed_Point[s].Modified_y,
                           s, Seed_Point[s].Modified_x,
                           Seed_Point[s].Modified_y, SuperPixelsNum);
...
inline void HQSGTRD::ProduceSuperpixelLable(int x, int y, int m, int SeedX,
                                            int SeedY, int& SuperPixelsNum) {
  ...
  if (PixelMatrix[x][y].SuperpixelLabel != m &&
      PixelMatrix[x][y].SuperpixelLabel != -1) {
    return;
  }
...
int HQSGTRD::Seed_Point_Generation(int k, Mat img) {
  ...
  for (int i = 0; i < rstrips; i++) {
    ...
    // int j = cstrips-1;
    {
      ...
      if (seed_r >= img.rows) seed_r = img.rows - 1;
      if (seed_c >= img.cols) seed_c = img.cols - 1;
      ...
      if (i == 0 && j == cstrips - 1) {
        if ((img.cols - y) < c_interval / 2) HBSign = 1;
      }
      ...
      if (j == 0 && i == 0) {
        ...
        if (Seed_Point[idx].UpperVertex_x <
            Seed_Point[idx].RightVerticesOne_x) {
      ...
      if (j != 0 && i == rstrips - 1 && j != cstrips - 1) {
        if (Seed_Point[idx].LowerVertex_x >
            Seed_Point[idx].RightVerticesTwo_x) {
  ...
  for (int i = 0; i < idx; i++) {
    DDALine(Seed_Point[i].UpperVertex_x, Seed_Point[i].UpperVertex_y,
            Seed_Point[i].LeftVerticesOne_x, Seed_Point[i].LeftVerticesOne_y,
            img, 1);

    DDALine(Seed_Point[i].UpperVertex_x, Seed_Point[i].UpperVertex_y,
            Seed_Point[i].RightVerticesOne_x, Seed_Point[i].RightVerticesOne_y,
            img, 1);

    DDALine(Seed_Point[i].LeftVerticesOne_x, Seed_Point[i].LeftVerticesOne_y,
            Seed_Point[i].LeftVerticesTwo_x, Seed_Point[i].LeftVerticesTwo_y,
            img, 1);

    DDALine(Seed_Point[i].LeftVerticesTwo_x, Seed_Point[i].LeftVerticesTwo_y,
            Seed_Point[i].LowerVertex_x, Seed_Point[i].LowerVertex_y, img, 1);

    DDALine(Seed_Point[i].RightVerticesOne_x, Seed_Point[i].RightVerticesOne_y,
            Seed_Point[i].RightVerticesTwo_x, Seed_Point[i].RightVerticesTwo_y,
            img, 1);

    DDALine(Seed_Point[i].RightVerticesTwo_x, Seed_Point[i].RightVerticesTwo_y,
            Seed_Point[i].LowerVertex_x, Seed_Point[i].LowerVertex_y, img, 1);
  ...
  {
    dQNum = 0;
    differential_Quotient dQ =
        new differentialQuotient[static_cast<int>(spixel_size) * 2];
    for (int i = Seed_Point[n].LeftVerticesOne_x;
         i <= Seed_Point[n].LeftVerticesTwo_x; i++) {
      for (int j = Seed_Point[n].LeftVerticesOne_y;
           j <= Seed_Point[n].RightVerticesOne_y; j++) {
        Csign = 0;
        ...
        for (int n = i - 1; n <= i + 1; n++) {
          for (int m = j - 1; m <= j + 1; m++) {
            if (PixelMatrix[n][m].ClassSign != PixelMatrix[i][j].ClassSign) {
              Csign++;
            }
          }
        }
        ...
        dQ[dQNum].y = j;
        dQ[dQNum].dQNum =
            (float)sqrt((i - Seed_Point[n].x) * (i - Seed_Point[n].x) +
                        (j - Seed_Point[n].y) * (j - Seed_Point[n].y));
        dQNum++;
      }
    }
    ...
    if (dQNum != 0) {
      ...
      dQNum = 0;
      for (int i = Seed_Point[n].LeftVerticesOne_x + 0;
           i <= Seed_Point[n].LeftVerticesTwo_x - 0; i++) {
        for (int j = Seed_Point[n].LeftVerticesOne_y + 0;
             j <= Seed_Point[n].RightVerticesOne_y - 0; j++) {
          if ((i - 1 < 0) || (i + 1 > PictureI)) {
            continue;
          }
          ...
          float A1, A1L, A1A, A1B, A2, A2L, A2A, A2B, A3, A3L, A3A, A3B, A4,
              A4L, A4A, A4B;

          float L = 0.0, A = 0.0, B = 0.0;

          A1L = (((float)PixelMatrix[i - 1][j - 1].L -
                  (float)PixelMatrix[i][j].L) -
                 ((float)PixelMatrix[i][j].L -
                  (float)PixelMatrix[i + 1][j + 1].L)) /
                2.0;

          A1A = (((float)PixelMatrix[i - 1][j - 1].a -
                  (float)PixelMatrix[i][j].a) -
                 ((float)PixelMatrix[i][j].a -
                  (float)PixelMatrix[i + 1][j + 1].a)) /
                2.0;

          A1B = (((float)PixelMatrix[i - 1][j - 1].b -
                  (float)PixelMatrix[i][j].b) -
                 ((float)PixelMatrix[i][j].b -
                  (float)PixelMatrix[i + 1][j + 1].b)) /
                2.0;

          A1 = (abs(A1L) + abs(A1A) + abs(A1B)) / 3.0;

          A2L =
              (((float)PixelMatrix[i - 1][j].L - (float)PixelMatrix[i][j].L) -
               ((float)PixelMatrix[i][j].L - (float)PixelMatrix[i + 1][j].L)) /
              2.0;
          A2A =
              (((float)PixelMatrix[i - 1][j].a - (float)PixelMatrix[i][j].a) -
               ((float)PixelMatrix[i][j].a - (float)PixelMatrix[i + 1][j].a)) /
              2.0;
          A2B =
              (((float)PixelMatrix[i - 1][j].b - (float)PixelMatrix[i][j].b) -
               ((float)PixelMatrix[i][j].b - (float)PixelMatrix[i + 1][j].b)) /
              2.0;

          A2 = (abs(A2L) + abs(A2A) + abs(A2B)) / 3.0;

          A3L = (((float)PixelMatrix[i - 1][j + 1].L -
                  (float)PixelMatrix[i][j].L) -
                 ((float)PixelMatrix[i][j].L -
                  (float)PixelMatrix[i + 1][j - 1].L)) /
                2.0;

          A3A = (((float)PixelMatrix[i - 1][j + 1].a -
                  (float)PixelMatrix[i][j].a) -
                 ((float)PixelMatrix[i][j].a -
                  (float)PixelMatrix[i + 1][j - 1].a)) /
                2.0;

          A3B = (((float)PixelMatrix[i - 1][j + 1].b -
                  (float)PixelMatrix[i][j].b) -
                 ((float)PixelMatrix[i][j].b -
                  (float)PixelMatrix[i + 1][j - 1].b)) /
                2.0;
          ...
          A4L =
              (((float)PixelMatrix[i][j - 1].L - (float)PixelMatrix[i][j].L) -
               ((float)PixelMatrix[i][j].L - (float)PixelMatrix[i][j + 1].L)) /
              2.0;

          A4A =
              (((float)PixelMatrix[i][j - 1].a - (float)PixelMatrix[i][j].a) -
               ((float)PixelMatrix[i][j].a - (float)PixelMatrix[i][j + 1].a)) /
              2.0;

          A4B =
              (((float)PixelMatrix[i][j - 1].b - (float)PixelMatrix[i][j].b) -
               ((float)PixelMatrix[i][j].b - (float)PixelMatrix[i][j + 1].b)) /
              2.0;
  ...
  /*	img.at<Vec3b>(SEED[i].Modified_x, SEED[i].Modified_y)[0] =
     DrawcolourRed[i % 255]; img.at<Vec3b>(SEED[i].Modified_x,
     SEED[i].Modified_y)[1] = DrawcolourBlue[i % 255];
            img.at<Vec3b>(SEED[i].Modified_x, SEED[i].Modified_y)[2] =
     DrawcolourGreen[i % 255];
            */
  ...
  /*	cv::Point start;
        ...
        {
        cv::Point start = cv::Point(SEED[i].UpperVertex_y,
     SEED[i].UpperVertex_x); //直线起点 cv::Point end =
     cv::Point(SEED[i].LeftVerticesOne_y, SEED[i].LeftVerticesOne_x); //直线终点
        cv::line(img, start, end, cv::Scalar(0, 0, 0));
        }

        //	if (SEED[i].UpperVertex_x != 0 && SEED[i].RightVerticesOne_y !=
     img.cols - 1)
        {
        start = cv::Point(SEED[i].UpperVertex_y, SEED[i].UpperVertex_x);
     //直线起点 end = cv::Point(SEED[i].RightVerticesOne_y,
     SEED[i].RightVerticesOne_x);   //直线终点 cv::line(img, start, end,
     cv::Scalar(0, 0, 0));
        }
        ...
        {
        start = cv::Point(SEED[i].LeftVerticesOne_y, SEED[i].LeftVerticesOne_x);
     //直线起点 end = cv::Point(SEED[i].LeftVerticesTwo_y,
     SEED[i].LeftVerticesTwo_x);   //直线终点 cv::line(img, start, end,
     cv::Scalar(0, 0, 0));
        }


        //if (SEED[i].LowerVertex_x != img.rows - 1 && SEED[i].LeftVerticesTwo_y
     != 0)
        {

        start = cv::Point(SEED[i].LeftVerticesTwo_y, SEED[i].LeftVerticesTwo_x);
     //直线起点 end = cv::Point(SEED[i].LowerVertex_y, SEED[i].LowerVertex_x);
     //直线终点 cv::line(img, start, end, cv::Scalar(0, 0, 0));
        }

        //if (SEED[i].RightVerticesOne_y != img.cols - 1 &&
     SEED[i].RightVerticesTwo_y != img.cols - 1)
        {
        start = cv::Point(SEED[i].RightVerticesOne_y,
     SEED[i].RightVerticesOne_x); //直线起点 end =
     cv::Point(SEED[i].RightVerticesTwo_y, SEED[i].RightVerticesTwo_x);
     //直线终点 cv::line(img, start, end, cv::Scalar(0, 0, 0));
        }

        //	if (SEED[i].LowerVertex_x != img.rows - 1 &&
     SEED[i].RightVerticesTwo_y != img.cols - 1)
        {
        start = cv::Point(SEED[i].RightVerticesTwo_y,
     SEED[i].RightVerticesTwo_x); //直线起点 end =
     cv::Point(SEED[i].LowerVertex_y, SEED[i].LowerVertex_x);   //直线终点
        cv::line(img, start, end, cv::Scalar(0, 0, 0));
        }
  ...
  // int i = 0;
  {
    ...
    if (i % 2 == 1) {
      for (int m = DD; m <= AA; m++) {
        DDALine(Seed_Point[m].Modified_x, Seed_Point[m].Modified_y,
                Seed_Point[m - cstrips].Modified_x,
                Seed_Point[m - cstrips].Modified_y, img, 2);

        DDALine(Seed_Point[m].Modified_x, Seed_Point[m].Modified_y,
                Seed_Point[m - cstrips + 1].Modified_x,
                Seed_Point[m - cstrips + 1].Modified_y, img, 2);

        if (m != DD) {
          DDALine(Seed_Point[m].Modified_x, Seed_Point[m].Modified_y,
                  Seed_Point[m - 1].Modified_x, Seed_Point[m - 1].Modified_y,
                  img, 2);
        }
    ...
    /*	if (i % 2 == 1) {
                ...
                DDALine(SEED[m].Modified_x, SEED[m].Modified_y, SEED[m -
       cstrips].Modified_x, SEED[m - cstrips].Modified_y, img);

                DDALine(SEED[m].Modified_x, SEED[m].Modified_y, SEED[m - cstrips
       + 1].Modified_x, SEED[m - cstrips + 1].Modified_y, img);

                if (m != DD)
                {
                DDALine(SEED[m].Modified_x, SEED[m].Modified_y, SEED[m -
       1].Modified_x, SEED[m - 1].Modified_y, img);
                }
...
inline void HQSGTRD::DDALine(int x0, int y0, int x1, int y1, Mat img,
                             int sign) {
  if (x1 < x0) {
    int temp;
...
int HQSGTRD::Non_Flat_Regions_Generation(string path, int superpixel_num_K,
                                         int grid_interval_S, Mat img,
                                         int iteration_num,
                                         Mat& Finally_labelMat) {
  ...
  for (int i = 0; i < img.rows; i++) {
    for (int j = 0; j < img.cols; j++) {
      ...
      if (labelMat.at<int>(i, j) == -1) {
        PixelMatrix[i][j].NormL =
            double(PixelMatrix[i][j].L - MinL) / double(MaxL);

        PixelMatrix[i][j].NormA =
            double(PixelMatrix[i][j].a - MinA) / double(MaxA);

        PixelMatrix[i][j].NormB =
            double(PixelMatrix[i][j].b - MinB) / double(MaxB);

        PixelMatrix[i][j].NormX = double(i) / double(img.rows);

        PixelMatrix[i][j].NormY = double(j) / double(img.cols);

        PixelMatrix[i][j].NormG =
            double(PixelMatrix[i][j].g - minGNum) / double(maxGNum);
      }
    }
  }

  Iterative_Generation(labelMat, superpixel_num_K, grid_interval_S,
                       iteration_num, img);
  ...
  vector<cv::Vec2i> first_pixel(img.rows *
                                img.cols);  // 记录每个超像素第一个像素
      ...
      if (labelMat.at<int>(i, j) < 0) {
        ...
        for (int c = 0; c < count; c++) {
          for (int n = 0; n < 4; n++) {
            ...
            if (np[0] >= 0 && np[0] < img.rows && np[1] >= 0 &&
                np[1] < img.cols) {
              if (labelMat.at<int>(np.val) < 0 &&
                  cur_label == old_labelMat.at<int>(np.val)) {
                vec3[count] = np;
...
inline void HQSGTRD::Iterative_Generation(cv::Mat& labelMat, int numseeds,
                                          int grid_interval_S, int itr_times,
                                          cv::Mat img) {
  double WL = 1.0 / 5.0;
  double Wa = 1.0 / 5.0;
  double Wb = 1.0 / 5.0;
  ...
  for (int k = 0; k < numseeds; k++) {
    ...
    PixelMatrix[Seed_Point[k].Modified_x][Seed_Point[k].Modified_y]
        .SuperpixelLabel = k;
    coC[k].cb = PixelMatrix[Seed_Point[k].Modified_x][Seed_Point[k].Modified_y]
                    .b;  // 种子点颜色值
    coC[k].ca = PixelMatrix[Seed_Point[k].Modified_x][Seed_Point[k].Modified_y]
                    .a;  // 种子点颜色值
    coC[k].cL = PixelMatrix[Seed_Point[k].Modified_x][Seed_Point[k].Modified_y]
                    .L;  // 种子点颜色值
    coC[k].cg =
        PixelMatrix[Seed_Point[k].Modified_x][Seed_Point[k].Modified_y].g;
  }

  cv::Mat distvec(
      PictureI + 1, PictureJ + 1, CV_64F,
      cv::Scalar(DBL_MAX));  // distvec为rows*cols的64为float类型的矩阵
  ...
  cv::Mat SolidCircle(
      Circle_diameter, Circle_diameter, CV_8UC1,
      cv::Scalar(0));  // 直径*直径大小的矩阵，初始为0，8位无符号整型

  cv::circle(SolidCircle, cv::Point2i(Circle_radius, Circle_radius),
             Circle_radius, cv::Scalar(1),
             -1);  // 画圆，圆心（半径，半径），半径为半径
  ...
  for (int itr = 0; itr < itr_times; itr++) {
    ...
    if (itr >= 0) {
      for (int k = 0; k < numseeds; k++) {
        ...
        for (int i = 0; i < Circle_diameter; i++) {
          for (int j = 0; j < Circle_diameter; j++) {
            ...
            if (SolidCircle.at<uchar>(i, j) == 1 && r > -1 && r <= PictureI &&
                c > -1 && c <= PictureJ) {
              if (labelMat_sign.at<int>(r, c) != -1) {
                continue;
              }
              double dL = (centL - PixelMatrix[r][c].NormL) *
                          (centL - PixelMatrix[r][c].NormL);

              double da = (centA - PixelMatrix[r][c].NormA) *
                          (centA - PixelMatrix[r][c].NormA);

              double db = (centB - PixelMatrix[r][c].NormB) *
                          (centB - PixelMatrix[r][c].NormB);

              double ds = (centX - PixelMatrix[r][c].NormX) *
                              (centX - PixelMatrix[r][c].NormX) +
                          (centY - PixelMatrix[r][c].NormY) *
                              (centY - PixelMatrix[r][c].NormY);
              ...
              double dist = WL * dL + Wb * db + Wa * da + Wg * dg + Ws * ds;
      ...
      Weights_Adjustment(labelMat_sign, labelMat, WL, Wa, Wb, Wg, Ws, coC,
                         numseeds);
...
inline void HQSGTRD::Weights_Adjustment(cv::Mat labelMat_sign, cv::Mat Label,
                                        double& WL, double& Wa, double& Wb,
                                        double& Wg, double& Ws,
                                        centroid_Cluster coC, int numseeds) {
  double SWL = 0.0;
  double SWa = 0.0;
  double SWb = 0.0;
  double SWg = 0.0;
  ...
  WL = pow(nSWL, -gama) /
       (pow(nSWL, -gama) + pow(nSWa, -gama) + pow(nSWb, -gama) +
        pow(nSWg, -gama) + pow(nSWs, -gama));
  Wa = pow(nSWa, -gama) /
       (pow(nSWL, -gama) + pow(nSWa, -gama) + pow(nSWb, -gama) +
        pow(nSWg, -gama) + pow(nSWs, -gama));
  Wb = pow(nSWb, -gama) /
...
inline void HQSGTRD::Update_Seed_Point(cv::Mat Label, cv::Mat img_lab,
                                       int num_region, centroid_Cluster coC,
                                       int* spixel_size) {
...
void HQSGTRD::Superpixel_Combination(cv::Mat labelMat) {
      ...
      if (labelMat.at<int>(i, j) < 0) {
        ...
        for (int c = 0; c < count; c++) {
          for (int n = 0; n < 4; n++) {
            ...
            if (np[0] >= 0 && np[0] < labelMat.rows && np[1] >= 0 &&
                np[1] < labelMat.cols) {
              if (labelMat.at<int>(np.val) < 0 &&
                  cur_label == old_labelMat.at<int>(np.val)) {
                vec[count] = np;
  ...
  CombineSuperpixelsPriority(NumCondition, CurrentNumberSuperpixels,
                             CurrentSeedNum, statisticsSuperPixle);
  ...
  CombineSuperpixelsPriorityS(NumCondition, CurrentNumberSuperpixels,
                              CurrentSeedNum, statisticsSuperPixle1);
}

inline void HQSGTRD::number_Statistical_SuperpixelsLable(
    int CNum, Nodem NumberSuperPixle) {
  int arr[] = {-1};
  int size = 1;
  ...
  for (int i = 0; i <= PictureI; i++) {
    for (int j = 0; j <= PictureJ; j++) {
      ...
      NumberSuperPixle[Superpixel_Label[i][j].Lable].sign =
          Superpixel_Label[i][j].Lable;
      ...
      // NumberSuperPixle[MarkM[i][j].Lable].TotalGray =
      // NumberSuperPixle[MarkM[i][j].Lable].TotalGray + PixelMatrixNew[i][j].z;
      NumberSuperPixle[Superpixel_Label[i][j].Lable].TotalL =
          NumberSuperPixle[Superpixel_Label[i][j].Lable].TotalL +
          PixelMatrix[i][j].L;
      NumberSuperPixle[Superpixel_Label[i][j].Lable].TotalA =
          NumberSuperPixle[Superpixel_Label[i][j].Lable].TotalA +
          PixelMatrix[i][j].a;
      NumberSuperPixle[Superpixel_Label[i][j].Lable].TotalB =
          NumberSuperPixle[Superpixel_Label[i][j].Lable].TotalB +
          PixelMatrix[i][j].b;
    }
  }
...
inline void HQSGTRD::CombineSuperpixelsPriority(int countNum,
                                                int& NumberSuperpixels,
                                                int length,
                                                Nodem NumberSuperPixle) {
  priorityQueue pQ;
  ...
  while (true) {
    ...
    // cout << NumberSuperPixle[MarkM[i][j].Lable].totalNum <<"   "<<
    // MarkM[i][j].Lable << "\n";
    ...
    for (int nb = 0; nb < nborhood.size(); nb++) {
      ...
      float aversgeLC = NumberSuperPixle[nborhood[nb]].TotalL /
                        NumberSuperPixle[nborhood[nb]].totalNum;
      float aversgeAC = NumberSuperPixle[nborhood[nb]].TotalA /
                        NumberSuperPixle[nborhood[nb]].totalNum;
      float aversgeBC = NumberSuperPixle[nborhood[nb]].TotalB /
                        NumberSuperPixle[nborhood[nb]].totalNum;

      float aversgeXC = NumberSuperPixle[nborhood[nb]].AX /
                        NumberSuperPixle[nborhood[nb]].totalNum;
      float aversgeYC = NumberSuperPixle[nborhood[nb]].AY /
                        NumberSuperPixle[nborhood[nb]].totalNum;
      ...
      TempC = sqrt((aversgeL - aversgeLC) * (aversgeL - aversgeLC) +
                   (aversgeA - aversgeAC) * (aversgeA - aversgeAC) +
                   (aversgeB - aversgeBC) * (aversgeB - aversgeBC));

      float AD = sqrt((aversgeX - aversgeXC) * (aversgeX - aversgeXC) +
                      (aversgeY - aversgeYC) * (aversgeY - aversgeYC));
...
inline void HQSGTRD::CombineSuperpixelsPriorityS(int countNum,
                                                 int& NumberSuperpixels,
                                                 int length,
                                                 Nodem NumberSuperPixle) {
  priorityQueue pQ;
...
inline void HQSGTRD::FindNeighborhood(int x, int y, int lable,
                                      vector<int>& nborhood, int RM) {
  if (Superpixel_Label[x][y].Lable != lable) {
    vector<int>::iterator it =
        find(nborhood.begin(), nborhood.end(), Superpixel_Label[x][y].Lable);
...
inline void HQSGTRD::mergeSuperpixelsLable(int x, int y, int PreLabel,
                                           int AfterLabel) {
